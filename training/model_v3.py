...
class TargetNet(nn.Module):
    ...
    def forward(self, data):
        ...
        data.x, data.edge_attr, _ = self.conv3(data.x, data.edge_index, data.edge_attr, None, data.batch)
        data = self.resnet(data)
        #print('TARGETNET OUTPUT:')
        #print(data.x, data.edge_attr)
        
        return data

class AlphaDock(nn.Module):
    def __init__(self, ligand_model, target_model, hidden_dim, n_gaussians, dropout_rate=0.15, nodes_per_target=None, dist_threshold=1000):
        ...
        self.dist_threhold = dist_threshold

    def forward(self, data_ligand, data_target, y=None):
        #print('AlphaDock Inputs')
        #print(self.ligand_model)
        ...
        #print('\nTARGETNET OUTPUT h_t_pos:')
        #print(pi, sigma, mu, dist, atom_types, bond_types)
        #print(mu, sigma)
